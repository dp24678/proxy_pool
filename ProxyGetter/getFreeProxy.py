...
class GetFreeProxy(object):
    ...
    def freeProxyThird(page=1):
        """
        旗云代理 http://www.qydaili.com/free/?action=china
        :param page: 页码数
        :return:
        """
        url_list = [
            'http://www.qydaili.com/free/?action=china&page={page}',  # 国内代理
            'http://www.qydaili.com/free/?action=unchina&page={page}'  # 国外代理
        ]
        for url in url_list:
            html_tree = getHtmlTree(url.format(page=page))
            try:
                tr_list = html_tree.xpath('//tbody/tr')
                for tr in tr_list:
                    tds = tr.xpath('./td/text()')
                    yield "{}:{}".format(tds[0],tds[1])
            except Exception as e:
                pass
    ...
    def freeProxySixth():
        """
        http://www.66ip.cn/nm.html  api提取接口
        :return:
        """
        url = 'http://www.66ip.cn/nmtq.php?getnum=300&isp=0&anonymoustype=4&start=&ports=&export=&ipaddress=&area=0&proxytype=1&api=66ip'
        request = WebRequest()
        try:
            res = request.get(url, timeout=10).text
            ip_regex = r"([\d.:]*?)<br"
            IP_list = re.findall(ip_regex,res)
            for ip in IP_list:
                yield ip
        except Exception as e:
            pass
